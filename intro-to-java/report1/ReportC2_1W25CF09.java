...
public class ReportC2_1W25CF09 {
  public static void main(String args[]) {
    ...
    while (player_hp > 0 && monster_hp > 0) {
      ...
      boolean playerCrit = rand.nextInt(5) == 0;
      if (playerCrit) {
        monster_rand_val *= 2;
      }
      monster_hp -= monster_rand_val;
      System.out.println("You inflicted " + monster_rand_val + " damage! " +
                         (playerCrit ? "CRITICAL HIT! " : "") +
                         "The monster has " + monster_hp + "HP left");

      int player_rand_val = rand.nextInt(max_rand_val);
      boolean monsterCrit = rand.nextInt(5) == 0;
      if (monsterCrit) {
        player_rand_val *= 2;
      }
      player_hp -= player_rand_val;
      System.out.println("The monster inflicted " + player_rand_val +
                         " damage! " + (monsterCrit ? "CRITICAL HIT! " : "") +
